...
describe 'w_apache::deploy' do
	context 'with default setting' do
		...
		let(:web_apps) do
		  [
		  	{ vhost: { main_domain: 'examplewebsite.com', docroot: 'examplewebsite.com' } }
		  ]
	  end

		let(:deploy) do
		  	{ enabled: true,  repo_ip: '9.9.9.9', repo_domain: 'git.examplewebsite.com', repo_url: 'https://git.examplewebsite.com/repo.git'}
	  end

	  let(:chef_run) do
	    ChefSpec::SoloRunner.new do |node|
	    	node.set['w_common']['web_apps'] = web_apps
	    	node.set['w_apache']['deploy'] = deploy
				node.set['w_memcached']['ips'] = ['127.0.0.1']
	    end.converge(described_recipe)
	  end

	  before do
	    stub_command("cat /websites/examplewebsite.com/.git/config | grep https://git.examplewebsite.com/repo.git").and_return(false)
	    stub_command("cat /websites/examplewebsite.com/.git/config | grep 'fileMode = false'").and_return(false)
	  ...
	  it 'runs a execute git init with attributes' do
	    expect(chef_run).to run_execute('git init').with(cwd: '/websites/examplewebsite.com', user: 'www-data', group: 'www-data', creates: '/websites/examplewebsite.com/.git/HEAD')
	  end

	  it 'runs a execute git remote add origin url with attributes' do
	    expect(chef_run).to run_execute('git remote add origin https://git.examplewebsite.com/repo.git').with(cwd: '/websites/examplewebsite.com', user: 'www-data', group: 'www-data')
	  end

	  it 'doesn not run a execute git remote add origin url when it already exists' do
	  	stub_command("cat /websites/examplewebsite.com/.git/config | grep https://git.examplewebsite.com/repo.git").and_return(true)
	    expect(chef_run).to_not run_execute('git remote add origin https://git.examplewebsite.com/repo.git').with(cwd: '/websites/examplewebsite.com', user: 'www-data', group: 'www-data')
	  end
	end
end